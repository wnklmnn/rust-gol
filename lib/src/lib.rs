...
impl GoLField {
    ...
    pub fn new(width: u32, height: u32) -> GoLField {
        ...
        GoLField {
            field_data: field.into_boxed_slice(),
            width,
            height,
        }
    }
...
impl std::fmt::Display for GoLField {
    fn fmt(&self, form: &mut std::fmt::Formatter) -> Result<(), std::fmt::Error> {
        for h in 0..self.height {
            for w in 0..self.width {
                if self.get_cell(w, h) == CellState::Alive {
                    write!(form, "X")?;
...
mod tests {
    ...
    fn iter_simple_deadcell() {
        ...
        assert_eq!(field.field_data.deref(), &[8_u8, 0_u8]);
        let new_field = field.calc_next_iteration(&super::EdgeBehavior::DeadCells);
        assert_eq!(new_field.field_data.deref(), &[0_u8, 0_u8]);
    }
    ...
    fn iter_simple_static_block_deadcell() {
        ...
        assert_eq!(field.field_data.deref(), &[216_u8, 0_u8]);
        let new_field = field.calc_next_iteration(&super::EdgeBehavior::DeadCells);
        assert_eq!(new_field.field_data.deref(), &[216_u8, 0_u8]);
    }
    ...
    fn iter_blink_deadcell() {
        ...
        assert_eq!(field.field_data.deref(), &[73_u8, 0_u8]); //0100_1001_0000_0000
        let new_field = field.calc_next_iteration(&super::EdgeBehavior::DeadCells);
        /*
        000
        111
        ...
        0001_1100_0000_0000
         */
        assert_eq!(new_field.field_data.deref(), &[28_u8, 0_u8]);
    }
    ...
    fn corpus_static_block_over_edge() {
        ...
        */
        ...
        assert_eq!(field.field_data.deref(), &[0xA2, 0x80]);

        let new_field = field.calc_next_iteration(&super::EdgeBehavior::Wrapping);
        assert_eq!(new_field.field_data.deref(), &[0xA2, 0x80]);
    }
    ...
    fn iter_blink_corpus() {
        ...
        assert_eq!(field.field_data.deref(), &[68_u8, 4_u8]); //0100_1001_0000_0000

        let new_field = field.calc_next_iteration(&super::EdgeBehavior::Wrapping);
        assert_eq!(new_field.field_data.deref(), &[224_u8, 0_u8]);

        let new_field = new_field.calc_next_iteration(&super::EdgeBehavior::Wrapping);
        assert_eq!(new_field.field_data.deref(), &[68_u8, 4_u8]); //0100_1001_0000_0000
    }
